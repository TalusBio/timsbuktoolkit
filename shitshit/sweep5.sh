#!/usr/bin/env bash
# Full-search check of the configs the isotope fixture ranks best.
set -u
cd "$(dirname "$0")/.."
...
OUT=shitshit/sweep5_results.tsv
MEM_LIMIT_MB=8192
printf "tag\tstatus\tpeak_fp_mb\twall_s\tidx_s\tq01\tq05\tno_signal\n" > "$OUT"
...
" 2>/dev/null)
  ...
  printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" "$tag" "$status" "$peak" "$wall" "${idx:-}" "$q01" "$q05" "${nosig:-}" >> "$OUT"
}

run_point both_im1        "im_tol = { Pct = 1.0 }" "im_tol = { Pct = 1.0 }"
run_point nt_both_im1     "transitive = false\nim_tol = { Pct = 1.0 }" "transitive = false\nim_tol = { Pct = 1.0 }"
run_point ms1_im0p5       "im_tol = { Pct = 0.5 }" ""
echo SWEEP_DONE >> "$OUT"
