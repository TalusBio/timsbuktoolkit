...
pub struct ScoringWorker {
    ...
    pub isotope_query: Option<timsquery::OwnedTarget<IonAnnot>>,
}
...
/// don't drift.
#[inline]
fn gate_expected_fragments(query: &RefQuery<'_>) -> Result<(), SkipReason> {
    if query.fragment_count() == 0 {
...
///
/// Removes lower-ranked fragments from the collector and expected intensities,
/// keeping their fragment counts aligned.
...
pub fn filter_zero_intensity_ions<T: KeyLike + Default>(
    ...
    // Assert collector rows and expected intensities agree.
    assert_eq!(
...
impl<I: ScorerQueriable> Scorer<I> {
    ...
    fn execute_secondary_query(
        ...
        let isotope_query = worker
            .isotope_query
            .get_or_insert_with(OwnedTarget::empty_like);
...
impl<I: ScorerQueriable> Scorer<I> {
    ...
    fn build_calibrated_extraction_into(
        ...
        if let (Some(original_rt), Some(observed_rt)) = (original_rt, calibrated_rt) {
            let tolerance = calibration.get_tolerance(
                ...
                query.mobility_ook0(),
                LibraryRT(original_rt),
            );
            let observed_query = timsquery::AtObservedRt::new(query, observed_rt.0);
        ...
        Ok(super::apex_finding::CandidateMetadata {
            ...
            library_rt: original_rt.unwrap_or(f32::NAN),
                ...
                match calibration {
                    ...
                    None => {
                        ...
                        Ok(super::apex_finding::CandidateMetadata {
                            ...
                            handles: identity.handles,
                            source_id: identity.source_id,
                            library_rt: query.library_rt().unwrap_or(f32::NAN),
    ...
    pub fn prescore_partitioned_batch(
        ...
        let (_worker, heap, par_timings) = crate::utils::maybe_par::fold_reduce(
            flats,
            || {
                tracing::debug!(
                    target: "alloc_track",
                    ...
                    capacity_per_band,
                    max_frags,
                    "worker init: ScoringWorker + StratifiedCalibrantHeaps"
                );
                (
                    ScoringWorker::new(num_cycles, max_frags),
            ...
            |(mut worker, mut heap, mut t), (_idx, &flat)| {
                let q = get_item(flat);
                if !filter_fn(&q) {
                    return (worker, heap, t);
                }
                t.n_passed_filter += 1;
                match self.prescore(&q, &mut worker, &mut t) {
                    Ok(loc) => {
                        let cand = CalibrantCandidate {
                            score: loc.score,
                            apex_rt: ObservedRTSeconds(loc.retention_time_ms as f32 / 1000.0),
                            speclib_index: flat,
                            library_rt: LibraryRT(q.library_rt().unwrap_or(f32::NAN)),
                        };
