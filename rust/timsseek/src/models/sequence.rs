...
fn modification_to_mod(m: &mzcore::sequence::Modification) -> Option<Mod> {
    ...
    match simple.as_ref() {
        ...
        SimpleModificationInner::Database { id, .. } => {
            ...
            // UNIMOD accessions are numeric; a non-numeric CURIE is not
            // something `Mod::Unimod(u16)` can represent.
            match id.id() {
...
mod tests {
    ...
    /// `ParsedSequence` — including residues, so the ontology path cannot
    /// quietly disagree with the fast path about the peptide itself.
    #[test]
