...
| `TIMSSEEK_RESCORE_DASHBOARD` | `timsseek`, **only when built with `--features dashboard`** (opt-in, not in `default`) | unset (off) | Set to any value other than `0`/`false` to open the rescore dashboard TUI after results are written (Phase 6 already ran, so a dashboard left open — or killed — never costs a rerun). Skips silently, with a log warning, when stdout is not a terminal. **Setting this on a binary built without `--features dashboard` does nothing** — the feature gate compiles the check out entirely, there is no warning. Dev-only debugging aid, not part of the documented CLI surface. Full invocation: `cargo run -r --bin timsseek --features dashboard -- ...`. |
| `TIMSSEEK_RESCORE_DASHBOARD_SAMPLE` | `timsseek`, **only with `--features dashboard`** | 250000 | Rows the dashboard's histograms are built from. Set at or above the row count to make every histogram, clip range and per-feature AUC exact instead of sampled — slower, and only worth it when a sampled panel looks suspicious. Ignored unless the dashboard is on. |
