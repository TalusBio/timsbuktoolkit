...
| `TIMSSEEK_CALIB_DASHBOARD` | `timsseek_cli` | unset | Set to `1` to open the interactive RT-calibration dashboard. Does nothing unless the binary was built with `--features calib-dashboard` — on a plain build this var is never read. Not a terminal? The dashboard detaches with a warning and the search runs unattended, so it's also safe under `> file 2>&1` redirection or CI. Full invocation: `TIMSSEEK_CALIB_DASHBOARD=1 cargo run -r --bin timsseek --features calib-dashboard -- <args>`. |
| `CALIB_DASH_FRAME_BUDGET_MB` | `timsseek_cli` | `64` | Only read when `TIMSSEEK_CALIB_DASHBOARD=1` and the binary has `calib-dashboard` compiled in. Caps the dashboard's retained-frame slab (Phase 1 calibrant snapshots kept for scrubbing/replay); a longer search or a larger `n_calibrants` decimates more aggressively to stay under budget rather than growing unbounded. |
